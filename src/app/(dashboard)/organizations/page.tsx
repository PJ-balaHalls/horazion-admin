...
import { CreateEntityWizard } from '@/components/organizations/CreateEntityWizard';
...
export default function OrganizationsPage() {
  ...
  // MODO SPA PURO: Sem divs em volta, sem paddings. O Wizard assume o controlo absoluto do espaço.
  if (isCreating) {
    return (
      <CreateEntityWizard 
  ...
  return (
    <div className="p-10 max-w-7xl mx-auto space-y-10 animate-in fade-in">
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-4xl font-black tracking-tighter text-black">Grafo de Organizações</h1>
          <p className="text-gray-500 text-sm mt-2">Gestão de identidades corporativas e subsidiárias (B2B).</p>
        </div>
        <HzButton 
          onClick={() => setIsCreating(true)} 
          className="bg-[#E50000] hover:bg-red-700 text-white flex gap-2 items-center px-6 py-2 rounded-xl transition-colors shadow-md font-bold"
        >
      ...
      <div className="grid grid-cols-3 gap-6">
        {loading ? (
          ...
          <div key={ent.id} className="bg-white border border-gray-100 p-6 rounded-[24px] hover:shadow-xl hover:border-[#E50000] transition-all duration-300 group flex flex-col justify-between">
            <div>
              ...
              <h3 className="text-xl font-black truncate text-black">{ent.display_name}</h3>
              <p className="text-[10px] font-mono text-gray-400 mt-1 uppercase tracking-wider">{ent.slug}</p>
            </div>
            
            <HzButton variant="ghost" onClick={() => router.push(`/organizations/${ent.id}`)} className="w-full mt-6 justify-between border border-gray-100 text-gray-600 hover:text-black hover:bg-gray-50 font-bold">
